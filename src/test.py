...
class SST:
    def __init__(self):
        self.name = 'SST'
        self.label_to_names = {0: 'unlabeled', 1: 'kickboard'}
            ...
            while True:
                try:
                    ...
                    for j in range(len(self.test_probs)):
                        ...
                        pred = self.remap_lut[pred]
                        self.publish_cloud(pred, msg)
                    epoch_ind += 1
                    if epoch_ind == 1:
                        break
        finally:
    ...
    def publish_cloud(self, pred, msg):
        r = pred / 1.0
        g = pred / 1.0
        b = pred / 1.0
        ...
        rgb = np.left_shift(rgb[:, 0], 16) + np.left_shift(rgb[:, 1], 8) + np.left_shift(rgb[:, 2], 0)
        rgb = rgb.astype(np.uint32)
        msg_out = PointCloud2()
        msg_out.header = msg.header
        msg_out.fields = self.fields
        msg_out.is_bigendian = False
        msg_out.point_step = 16
        msg_out.row_step = 16 * len(pred)
        msg_out.height = 1
        msg_out.width = len(pred)
        msg_out.is_dense = True
        msg_out.data = struct.pack('%sf' % len(pred) * 4, *(rgb.flatten().tolist()))
    ...
    def listener(self):
        rospy.init_node('SST', anonymous=True)
        rospy.Subscriber("/ouster/points", PointCloud2, self.process_pointcloud, queue_size=1)
        rospy.spin()
